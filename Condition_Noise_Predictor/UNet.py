...
class NoisePred(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, timesteps):
        ...
        # y_e = y[:, 0:1, :, :].unsqueeze(1).repeat(1, x.shape[1], 1, 1,1).contiguous()
        ...
        # if not isinstance(n, NestedTensor):
        #     noisy_f_nest = nested_tensor_from_tensor(n)
        ...
        #todo 改进通道注意力 CBAM 筛选权重前百分之最大 y用传统算法的全焦图像去引导去噪
...
if __name__ == '__main__':
    ...
    print(net(torch.randn(8,2,3,128,128),torch.randn(8,3,128,128),torch.randint(0, 100, (8,)).long()))